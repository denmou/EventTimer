...
local function WilbaPrefabPostInit(inst)
    ...
    inst._eventTimer = function()
        ...
        if config and config.switch and (g_dlc_mode and config.dlc[g_dlc_mode]) and inst.trans_task_info then
            local waitTime = inst:TimeRemainingInTask(inst.trans_task_info)
